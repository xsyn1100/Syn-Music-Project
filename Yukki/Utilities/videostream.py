...
async def start_stream_video(message, file, title, mystic):
    ...
    if await is_active_chat(message.chat.id):
        ...
        final_output = await message.reply_photo(
            ...
            caption=(
                f"🎬<b>Video: </b> [Given Video Via Telegram]({link})\n\n👤<b>Diminta Oleh: </b>{message.from_user.mention} \n🚧<b>Antrian Ke:</b> <b>#{position}!</b>"
            ),
        ...
        if not await join_video_stream(message.chat.id, file, 720):
            return await mystic.edit(
                "Kesalahan bergabung Voice Chat. Pastikan **Obrolan Suara** diaktifkan."
            )
        ...
        await mystic.delete()
        cap = f"🎥<b>Bermain: </b>[Given Video Via Telegram]({link})\n👤**Diminta Oleh:** {message.from_user.mention}"
        final_output = await message.reply_photo(
            photo="Utils/Telegram.JPEG",
...
async def start_video_stream(
    ...
    if await is_active_chat(CallbackQuery.message.chat.id):
        ...
        if not await join_video_stream(
            ...
            return await CallbackQuery.message.reply_text(
                f"Kesalahan bergabung Voice Chat."
            )
