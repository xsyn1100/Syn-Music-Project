...
async def forceclose(_, CallbackQuery):
    ...
    if CallbackQuery.from_user.id != int(user_id):
        return await CallbackQuery.answer(
            "Kamu tidak boleh menutup ini.", show_alert=True
        )
...
async def admin_risghts(_, CallbackQuery):
    ...
    if not await is_active_chat(CallbackQuery.message.chat.id):
        return await CallbackQuery.answer(
            "Tidak ada yang main di voice chat.", show_alert=True
        )
    ...
    if command == "pausecb":
        ...
        await CallbackQuery.message.reply_text(
            f"▶️ Voicechat dijeda Oleh {CallbackQuery.from_user.mention}!",
    ...
    if command == "resumecb":
        if await is_music_playing(chat_id):
            return await CallbackQuery.answer(
                "Music Sudah Dilanjutkan.", show_alert=True
            )
        await music_on(chat_id)
        await resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            f"⏸ Voicechat Dilanjutkan Oleh {CallbackQuery.from_user.mention}!",
            reply_markup=audio_markup2,
        )
        await CallbackQuery.message.delete()
    ...
    if command == "stopcb":
        ...
        await CallbackQuery.message.reply_text(
            f"⏹ Voicechat Diakhiri Oleh {CallbackQuery.from_user.mention}!",
            reply_markup=audio_markup2,
        )
        await CallbackQuery.message.delete()
    ...
    if command == "skipcb":
        ...
        if Queues.is_empty(chat_id):
            ...
            await remove_active_video_chat(chat_id)
            await CallbackQuery.message.reply_text(
                f"Tidak ada musik lagi __Antrian__ \n\nMeninggalkan Voice Chat..Tombol Digunakan Oleh :- {CallbackQuery.from_user.mention}"
            )
            await stop_stream(chat_id)
            await CallbackQuery.message.delete()
            await CallbackQuery.answer(
                "Dilewati. Tidak ada lagi musik dalam antrian", show_alert=True
            )
            return
        else:
            ...
            if str(finxx) == "raw":
                ...
                final_output = await CallbackQuery.message.reply_photo(
                    ...
                    caption=f"<b>__Melewatkan Voice Chat__</b>\n\n🎥<b>**Mulai Bermain:**</b> {title} \n⏳<b>**Durasi:**</b> {duration_min} \n👤<b>**Diminta Oleh:** </b> {mention}",
                )
                await start_timer(
                    videoid,
                    duration_min,
            ...
            if str(finxx) == "s1s":
                ...
                mystic = await CallbackQuery.message.reply_text(
                    "Dilewati! Mohon Tunggu perubahan streaming Video..."
                )
                ...
                if int(quality) == 1080:
                    try:
                        ...
                        return await mystic.edit(
                            f"Kesalahan saat mengubah video stream.\n\nKemungkinan Masalah:- {e}"
                        )
                    ...
                    final_output = await CallbackQuery.message.reply_photo(
                        ...
                        reply_markup=InlineKeyboardMarkup(buttons),
                        caption=(
                            f"<b>__Melewatkan Video Chat__</b>\n\n👤**Diminta Oleh:** {mention}"
                        ),
                    )
                    await mystic.delete()
                else:
                    ...
                    final_output = await CallbackQuery.message.reply_photo(
                        ...
                        reply_markup=InlineKeyboardMarkup(buttons),
                        caption=(
                            f"<b>__Melewatkan Video Chat__</b>\n\n🎥<b>**Memulai Bermain Video:** </b>[{title[:25]}](https://www.youtube.com/watch?v={videoid}) \n👤**Diminta Oleh:** {mention}"
                        ),
                    )
                    os.remove(thumb)
                    await start_timer(
                ...
                await CallbackQuery.message.delete()
                await CallbackQuery.answer(
                    "Dilewati! Playlist Playing....", show_alert=True
                )
                mystic = await CallbackQuery.message.reply_text(
                    f"**{MUSIC_BOT_NAME} Fungsi Playlist**\n\n__Mengunduh Musik Selanjutnya Dari Playlist....__\n\nTombol Digunakan Oleh :- {CallbackQuery.from_user.mention}"
                )
                (
                    title,
                    duration_min,
                ...
                final_output = await CallbackQuery.message.reply_photo(
                    ...
                    reply_markup=InlineKeyboardMarkup(buttons),
                    caption=(
                        f"<b>__Melewatkan Voice Chat__</b>\n\n🎥<b>**Mulai Bermain:** </b>[{title[:25]}](https://www.youtube.com/watch?v={videoid}) \n⏳<b>**Durasi:**</b> {duration_min} Menit\n👤**Diminta Oleh:** {mention}"
                    ),
                )
                os.remove(thumb)
                await start_timer(
...
async def play_playlist(_, CallbackQuery):
    ...
    if smex == "third":
        ...
        if CallbackQuery.from_user.id != int(user_id):
            return await CallbackQuery.answer(
                "Ini bukan untukmu! Mainkan playlist anda sendiri", show_alert=True
            )
        _playlist = await get_playlist_names(user_id, type)
        ...
        return await CallbackQuery.answer("Kesalahan Dalam Playlist.")
    if CallbackQuery.message.chat.id not in db_mem:
        db_mem[CallbackQuery.message.chat.id] = {}
    try:
        ...
        if read1:
            return await CallbackQuery.answer(
                "Live Streaming Bermain...Berhenti untuk bermain playlist",
                show_alert=True,
            )
        else:
            pass
    ...
    if not _playlist:
        return await CallbackQuery.answer(
            f"Pengguna ini tidak memiliki playlist di Server.", show_alert=True
        )
    else:
        await CallbackQuery.message.delete()
        mystic = await CallbackQuery.message.reply_text(
            f"Memulai Playlist  {third_name}.\n\nDiminta Oleh:- {CallbackQuery.from_user.first_name}"
        )
        ...
        for shikhar in _playlist:
            ...
            if await is_active_chat(chat_id):
                ...
                if not await join_stream(chat_id, raw_path):
                    return await mystic.edit(
                        "Kesalahan bergabung Voice Chat. Pastikan Voice Chat diaktifkan."
                    )
                theme = await check_theme(chat_id)
